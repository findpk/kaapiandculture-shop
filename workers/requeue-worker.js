...
class RequeueWorker extends BaseWorker {
  ...
  async setDependencies() {
    const me = this
    try {
      this.messageHistoryAccessor = new MessageHistoryAccessor(me.dependencies, me.configs, me.context)
  ...
  async run() {
    const me = this
    try {
      let messages = me.messageHistoryAccessor.getByStatus(Enum.MessageHistoryStatus.Queued)
