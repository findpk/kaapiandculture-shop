...
class ScheduleEventUpdateNotificationsWorker extends BaseWorker {
  ...
  async setDependencies() {
    const me = this;
    try {
      this.lotEventsDbAccessor = new LotEventsDbAccessor(
        me.dependencies,
        me.configs,
        me.context,
      );
      this.notificationsDbAccessor = new NotificationsDbAccessor(
        me.dependencies,
        me.configs,
        me.context,
      );
      this.messageHistoryDbAccessor = new MessageHistoryDbAccessor(
        me.dependencies,
        me.configs,
        me.context,
      );
    ...
    try {
      ...
      for (let event of activeEvents) {
        ...
        let scheduledNotification =
          await me.notificationsDbAccessor.getByEventIdTypeAndDate(
            eventId,
            notificationType,
            currentDate,
          );
        if (!_.isEmpty(scheduledNotification)) {
          me.dependencies.pgp
            .tx("schedule-notification", async (t) => {
...
let scheduleEventUpdateNotificationsWorker =
  new ScheduleEventUpdateNotificationsWorker({}, {}, {});
