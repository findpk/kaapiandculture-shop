...
class ProductAccessor extends BaseHelper {
  ...
  async insert(payload) {
    ...
    try {
      ...
      return await me.pgp.one(
        "INSERT INTO products(id, data) VALUES($1, $2) RETURNING id",
        [id, data],
      );
    } catch (e) {
      throw e;
  ...
  async update(sku, data) {
    ...
    try {
      return await me.pgp.one(
        `UPDATE products set data = $2 where data->>'sku_id' = $1 RETURNING id`,
        [sku, data],
      );
    } catch (e) {
      throw e;
  ...
  async get(sku) {
    ...
    try {
      return await me.pgp.any(
        `SELECT data from products where data->>'sku_id' = $1`,
        [sku],
      );
    } catch (e) {
      throw e;
  ...
  async getAllByOffsetAndLimit(limit, offset = 0) {
    ...
    try {
      return await me.pgp.any(
        `SELECT data from products order by modified_date limit $1 offset $2`,
        [limit, offset],
      );
    } catch (e) {
      throw e;
