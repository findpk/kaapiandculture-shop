...
class NotificationsAccessor extends BaseHelper {
  ...
  async insert(payload) {
    ...
    try {
      ...
      return await me.pgp.one(
        "INSERT INTO notifications(id, data) VALUES($1, $2) RETURNING id",
        [id, data],
      );
    } catch (e) {
      throw e;
  ...
  async update(id, data) {
    ...
    try {
      return await me.pgp.one(
        "UPDATE notifications set data = $2 where id = $1 RETURNING id",
        [id, data],
      );
    } catch (e) {
      throw e;
  ...
  async get(id) {
    ...
    try {
      return await me.pgp.any(
        `SELECT notifications from message_history where id = $1`,
        [id],
      );
    } catch (e) {
      throw e;
  ...
  async getByStatus(status) {
    ...
    try {
      return await me.pgp.any(
        `SELECT * from notifications where data->>'status' = $1`,
        [status],
      );
    } catch (e) {
      throw e;
  ...
  async getByEventIdTypeAndDate(eventId, type, date) {
    ...
    try {
      return await me.pgp.any(
        `SELECT * from notifications where data->>'event_id' = $1 AND data->>'type' = $2 AND data->>'date' = $3`,
        [eventId, type, date],
      );
    } catch (e) {
      throw e;
