...
class ProductService extends BaseHelper {
    ...
    async getProduct(sku_id) {
        ...
        try {
            ...
            if (_.isEmpty(response)) throw new Errors.SKUNotFound();
    ...
    async updateProduct(sku_id, requestBody) {
        ...
        try {
            ...
            if (requestBody.tier_pricing) {
                ...
                let invalidWeights = _.difference(
                    providedWeights,
    ...
    async getAllProducts(query) {
        ...
        try {
            ...
            let response = await me.productAccessor.getAllByOffsetAndLimit(
                limit,
                offset,
            );
            let next = `/v1/product/fetchAll?page=${query.page + 1}&limit=${query.per_page}`;
            if (totalProducts < (query.page + 1) * query.per_page) next = null;
            return {
                total: totalProducts,
